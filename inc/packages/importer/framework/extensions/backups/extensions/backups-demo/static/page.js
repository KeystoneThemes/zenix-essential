...
jQuery(function($){
	...
	$(document).on('keyup keypress', '#wdb-demo-items-search-js' , function(e){		
		$('.filter-by-cat select').val('');
		var key = e.which;
		if(key == 13)  // the enter key code
		{
			wdb_filter_demo_content();			
		}else{
			wdb_filter_demo_content();			
		}
    ...
    function wdb_filter_demo_content(){
		$('.loadmore-wdb-demo').hide();
		var txt = $('#wdb-demo-items-search-js').val();
		var $all_items = $('.wdb-demo-item .wdb-demo-tpl-name');		
        if(txt.length > 1){
			$all_items.each(function(i, obj) {
				$( this ).parents('.wdb-demo-item').hide();			
				if($( this ).text().toLowerCase().includes( txt.toLowerCase() ) ){
					$( this ).parents('.wdb-demo-item').show();
				}else{
					$( this ).parents('.wdb-demo-item').hide();
				}				
			});
    ...
    //Category
    let $total_options = $('.fw-ext-backups-demo-item.wdb-demo-item');
    let $options_array = [];
    ...
    $(document).on('click' , '.loadmore-wdb-demo', function(e){
        e.preventDefault();        
        wdb_filter_demo_content();       
    });
});