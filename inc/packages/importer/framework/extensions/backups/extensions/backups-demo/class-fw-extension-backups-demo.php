...
class FW_Extension_Backups_Demo extends FW_Extension {
	...
	public function _admin_action_admin_menu() {
		if (
			...
			if(!apply_filters('wdb_sandbox_backups_demo_enable_menu', false)){
				return;
			}
			return;
		}
	...
	public function _display_page() {
		...
		if (function_exists('is_wpe_snapshot') && !is_wpe_snapshot()) {
			...
			sprintf(
				esc_html__('Theme Demo Content Install works only in %s.', 'fw'),
	...
	private function is_page() {
		...
		if(property_exists($current_screen, 'id') && 'zenix-theme_page_fw-backups-demo-content' === $current_screen->id){
