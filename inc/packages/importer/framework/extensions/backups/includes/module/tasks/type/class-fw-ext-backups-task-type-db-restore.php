...
class FW_Ext_Backups_Task_Type_DB_Restore extends FW_Ext_Backups_Task_Type {
	...
	private function get_tables() {
		 ...
		 * Fixes  /issues/2068#issuecomment-250196792
		 */
	...
	private function do_import(array $args, array $state) {
		 ...
		 */
		{
			...
			foreach ($search_replace as $search => $replace) {
				 ...
				 * Fixes  -Backups-Extension/issues/23
				 */
		 ...
		 * Fixes  -Backups-Extension/issues/27
		 */
		{
					...
					case 'row':
						...
						if (
							...
							if (isset($replace_option_names[ $line['data']['row']['option_name'] ])) {
								 ...
								 * Prevent 'Duplicate entry' error  -Backups-Extension/issues/34
								 *
								 * This happens when:
