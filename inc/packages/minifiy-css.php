...
abstract class zenix_MinificationSequenceFinder
{
    public $start_idx;
    public $end_idx;
    public $type;
...
class zenix_StringSequenceFinder extends zenix_MinificationSequenceFinder
{
...
class zenix_QuoteSequenceFinder extends zenix_MinificationSequenceFinder
{
...
$singlezenix_QuoteSequenceFinder = new zenix_QuoteSequenceFinder('\'');
$doublezenix_QuoteSequenceFinder = new zenix_QuoteSequenceFinder('"');
$blockCommentFinder = new zenix_StringSequenceFinder('/*', '*/');
$minificationStore = array();

function zenix_getNextMinificationPlaceholder(){
    global $minificationStore;
    return '<-!!-' . sizeof($minificationStore) . '-!!->';
}
function zenix_getNextSpecialSequence($string, $sequences){
...
function zenix_minify_CSS($css){
    global $minificationStore, $singlezenix_QuoteSequenceFinder, $doublezenix_QuoteSequenceFinder, $blockCommentFinder;
    $css_special_chars = array($blockCommentFinder, // CSS Comment
    $singlezenix_QuoteSequenceFinder, // single quote escape, e.g. :before{ content: '-';}
    $doublezenix_QuoteSequenceFinder); // double quote
    // pull out everything that needs to be pulled out and saved
    while ($sequence = zenix_getNextSpecialSequence($css, $css_special_chars)){
        switch ($sequence->type){
        case '/*': // remove comments
        $css = substr($css, 0, $sequence->start_idx) . substr($css, $sequence->end_idx);
        break;
        default: // strings that need to be preservered
        $placeholder = zenix_getNextMinificationPlaceholder();
