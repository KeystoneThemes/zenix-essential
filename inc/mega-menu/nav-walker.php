...
/**
    ...
    **/
    public function get_item_meta($item_id)
    {
        ...
        $is_mega_menu   = get_post_meta( $item_id, '_menu_item_wdb_mega_menu_enable', true );           
        $full_width     = get_post_meta( $item_id, '_menu_item_wdb_mega_menu_fullwidth', true );           
        $content_id     = get_post_meta( $item_id, '_menu_item_wdb_mega_menu_tpl_id', true );    
    ...
    {
        //wdb_mega_menu_post_item_mega_menu_enable
    ...
    public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
        ...
        $item_cls = get_post_meta( $item->ID, '_keystonethemes_theme_menu_item_cls', true );
        ...
        if ($is_megamenu_item) {
            $class_names .= ' wdb-mg-has-megamenu';
        }
        ...
        */
        ...
        if ( $this->elementor_settings['has_dropdown_arrow_icon'] && $depth === 0 && in_array('menu-item-has-children', $classes) || ($depth === 0 && $is_megamenu_item)) {
            $item_output .= zenix_render_elementor_icons( $this->elementor_settings['menu_down_icon'] );
        }
        
        if ( $this->elementor_settings['has_right_arrow_icon'] && $depth > 0 && in_array('menu-item-has-children', $classes)) {
            $item_output .= zenix_render_elementor_icons( $this->elementor_settings['menu_right_icon'] );
        }
    ...
    public function end_el(&$output, $item, $depth = 0, $args = array())
    {
       if ($depth === 0) {
                $item_meta = $this->get_item_meta($item->ID);
                if ($item_meta['is_mega_menu'] == 'on' && class_exists('Elementor\Plugin')) {   
                    $elementClass = 'mega_menu wdb-mg-megamenu-section ';
                    ...
                    if ($item_meta['content_id'] != '' && $elementor_page) {
                        ...
                        if( current_user_can('editor') || current_user_can('administrator') ) {    
                            $elementor_link = add_query_arg( [ 'action' => 'elementor' , 'wdb-edit' => 1 ], get_edit_post_link( $item_meta['content_id'] ) );
                            $no_content = sprintf('<div class="wdb--mega-menu-empty-content">
                                <h2>%s</h2>                           
                                <a target="_blank" href="%s" class="wdb--editemptybutton">
                            ...
                            esc_html__('No content found', 'zenix-essential'),
                            $elementor_link,
                            esc_html__('Edit Content', 'zenix-essential'),
                            );
                            $output .= $no_content;
                        }
                    }
