<?php
namespace ZenixEssentialApp\CustomControl;
...
class ImageSelector_Control extends Base_Data_Control {
	...
	public function enqueue() {
		$url = ZENIX_ESSENTIAL_URL.'/inc/custom-controls/css/';
		// Styles
		wp_enqueue_style('wdb-image-selector', $url.'image-selector.css', array(), '');
	}
					...
					<# _.each( data.options, function( options, value ) { 					
					 ...
					 <div class="elementor-wdb-col">
						...
						<label class="elementor-image-selector-label tooltip-target" for="<?php echo $control_uid; ?>" data-tooltip="{{ options.title }}" title="{{ options.title }}">
							...
							<img class="wdb-element-hover-preview" src="{{ options.preview_url }}" >
							<# } #>
						</label>
					</div>
					<# } ); #>
