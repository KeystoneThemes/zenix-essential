...
Class Zenix_Optimize_Assets{
 ...
 public function __construct(){
    ...
    // contact form 7
    if( zenix_option('ondemand_contact_form_7', true ) ){
    ...
    if(zenix_option( 'defer_js_and_css' , false) ){
 ...
 public function enqueue_scripts(){    
    ...
    wp_register_style( 'zenix-custom-icons' , ZENIX_CSS . '/custom-icons.min.css', null, ZENIX_ESSENTIAL_VERSION );
    wp_enqueue_style( 'zenix-custom-icons' );  
 }
 ...
 function info_optimize_theme_css($post_id)
  {
    ...
    if(!zenix_option('optimize_asset_enable')){
        return;
    }
    ...
    $deque               = [];
    $css_content         = '';
    $optimize_minify_css = zenix_option('optimize_asset_enable');
    ...
    zenix_new_directory($this->directory_path);
    foreach( $wp_styles->queue as $style ) {
        if( zenix_has_string_prefix( $style , $prefix ) && isset( $wp_styles->registered[ $style ] ) ){   
          $deque[$style] = $wp_styles->registered[$style]->src;             
        }             
    }
    
    foreach( $wp_styles->registered as $key => $style ) {          
        if( zenix_has_string_prefix( $key , $prefix ) ){                 
            $deque[$key] = $style->src;
        }             
    }
    ...
    if( $css_content !='' ){
        if($optimize_minify_css && 1==2){
            $wp_filesystem->put_contents($dir_path , zenix_minify_CSS( $css_content ) , 755 );
        }else{
  ...
  function enqueue_pure_styles() { 
    ...
    if( file_exists( $dirpath ) ){       
        ...
        foreach( $wp_styles->queue as $style ) { 
            ...
            if( zenix_has_string_prefix( $style , $prefix ) ){   
        ...
        wp_enqueue_style('zenix-styles-'.get_queried_object_id(), $path);
    }
