...
Class zenix_Custom_Fonts_Family{
    
    public $elementor_local_font = [];
    public $configs              = [];
    public $font_group_key       = 'zenix-custom';
    public $font_group_label     = 'zenix';
    public $meta_key             = 'wdb_custom_fonts';
    public $post_type            = 'wdb-custom-font';
    
    
    public function __construct(){ 
    
        $this->configs = zenix_get_config_value_by_name('fonts');
        ...
        add_filter( 'zenix_custom_webfonts' , [ $this, 'zenix_custom_webfonts_from_theme' ] , 4 );
        add_filter( 'elementor/document/urls/preview' , [ $this, 'zenix_elementor_editor_url' ] , 4 );
        add_filter( 'elementor/document/urls/wp_preview' , [ $this, 'zenix_elementor_editor_url' ] , 4 );
    ...
    public function register_sub_menu_post() { 
        add_submenu_page( 'wdb-zenix-theme-parent' , 'Custom Fonts' , 'Custom Fonts' , 'manage_options' , "edit.php?post_type=$this->post_type" );      
    }
    ...
    function custom_post_type(){
        ...
        register_post_type($this->post_type,
          array(
            'labels'      => array(
              'name'          => __('Custom Fonts', 'ZENIX_ESSENTIAL'),
              'singular_name' => __('Custom Font', 'ZENIX_ESSENTIAL'),
            ),
              'public'              => true,
    ...
    public function editor_after_save( $id , $editor_data ){  
        try {
            $url = add_query_arg( 'wdb-edit', 1 , get_the_permalink( $id ));       
           // get_headers( $url );
        }catch(\Exception $e) {}      
    } 
    ...
    public function zenix_elementor_editor_url( $url ){
      return add_query_arg( 'wdb-edit', 1 , $url ); 
    }
    
    function zenix_custom_webfonts_from_theme( $return_fonts ){
           
        $body_font_typho      = zenix_option('opt-tabbed-general');
    ...
    public function get_custom_font_from_user(){
        ...
        foreach($latest_posts as $item){
            ...
            $variation = get_post_meta( $item->ID , 'zenix_custom_fonts_options', true);
