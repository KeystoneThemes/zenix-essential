...
elementor.hooks.addAction('panel/open_editor/widget/accordion', function (panel, model, view) {
    var paccordion_elements = panel.$el.find('.elementor-control-type-repeater').find('.elementor-control-wdb_title_icon,.elementor-control-wdb_content_icon');
    panel.$el.find('[data-setting="_skin"]').find(':selected').text() == 'WDB' ? paccordion_elements.show() : paccordion_elements.hide();
    ...
    elementor.channels.editor.on('change', (e) => {
        if (e.$el.find('[data-setting="_skin"]').length) {
            var accordion_elements = e.$el.siblings('.elementor-control-type-repeater').find('.elementor-control-wdb_title_icon,.elementor-control-wdb_content_icon');

            e.$el.find('[data-setting="_skin"]').find(':selected').text() == 'WDB' ? accordion_elements.show() : accordion_elements.hide();
            if (e.$el.find('[data-setting="_skin"]').find(':selected').text() == 'WDB Style Two') {
...
elementor.hooks.addAction('panel/open_editor/widget/zenix--testimonial', function (panel, model, view) {
